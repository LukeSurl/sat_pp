...
def main():
    #initialisation
    initial_choice = ""
    while initial_choice not in ["Y","N"]:
        os.system('clear')
        initial_choice = initial().upper()
        if initial_choice == "Y":
            ...
            current_pickle = "/group_workspaces/jasmin/geoschem/local_users/lsurl/sat_pp/20140301-20141231"
            current_emfiles = ['/group_workspaces/jasmin/geoschem/local_users/lsurl/runs/geosfp_025x03125_nochem_2014/','_20140301-20141231']
            (NDVI_data_all,indv_data_all,binn_data_all) = load_new_pickles_all(current_pickle,verbose=True)
    ...
    while quit_main == False:
        os.system('clear')
        top_level_menu_choice = top_level_menu(current_pickle,current_emfiles)
        
        if top_level_menu_choice.upper() == quit_letter: #quit
            quit_main = True
        
        elif top_level_menu_choice.upper() == "P": #change pickle
        ...
        elif top_level_menu_choice.upper() == "E": #change emissions
            (changed,current_emfiles) = change_emfiles(current_emfiles)
            #currently no loading of the emisisons files are done here
        
        elif top_level_menu_choice.upper() == "X": #change options
            datesbefore = (startdate,enddate)
            (north,south,east,west,xdim,ydim,startdate,enddate) = global_options(north,south,east,west,xdim,ydim,startdate,enddate)
            if datesbefore != (startdate,enddate): #if time selection has changed, reselect data.
                print "Start and/or end time has changed. Selecting data..."                
                (lat,lon,sat_VC,sat_DVC,geos_VC,time,country,state,index_map) = time_select(startdate,enddate,time,lat,lon,sat_VC,sat_DVC,geos_VC,time,country,state,index_map)
                
        elif top_level_menu_choice.upper() == "R": #reload pickle
        ...
        elif top_level_menu_choice.upper() == "G": #geographic selection option
        ...
        elif top_level_menu_choice.upper() == "1": #Binned data
        ...
        elif top_level_menu_choice.upper() == "2": #Indiv data
